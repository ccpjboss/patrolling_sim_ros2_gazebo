...
import os
#dirname = rospkg.RosPack().get_path('patrolling_sim_ros2_gazebo')
dirname = '.'
...
def run_experiment(MAP, NROBOTS, INITPOS, ALG_SHORT, LOC_MODE, NAV_MODULE, GWAIT, COMMDELAY, TERM, TIMEOUT, CUSTOM_STAGE, SPEEDUP, USE_RVIZ):
    ...
    cmd_monitor = 'ros2 run patrolling_sim_ros2_gazebo monitor '+MAP+' '+ALG_SHORT+' '+NROBOTS + ' --ros-args -p goal_reached_wait:=' + \
        str(float(GWAIT))+' -p communication_delay:='+str(COMMDELAY) + \
        ' -p navigation_module:='+NAV_MODULE+' -p initial_positions:='+INITPOS
    ...
    gcmd = 'gnome-terminal '
    cmd = 'bash -c \'ros2 launch patrolling_sim_ros2_gazebo multi_stop_simulation_launch.py map_path:=' + \
        MAP+' n_robots:='+str(NROBOTS)+' use_rviz:='+str(USE_RVIZ)
    cmd = cmd + "'"
    print(cmd)
    gcmd = gcmd + ' --tab -e "'+cmd+'" '
    gcmd = gcmd + '&'
    ...
    for i in range(0, int(NROBOTS)):
        ...
        if (ALG_SHORT == 'MSP'):
            cmd = ('bash -c \'ros2 run patrolling_sim_ros2_gazebo '+ALG+' 0 ' + MAP + ' ' + str(i)+' MSP/'+MAP+'/'+MAP+'_'+str(NROBOTS)+'_'+str(i)+' --ros-args -p goal_reached_wait:='+str(float(GWAIT)) +
                   ' -p communication_delay:='+str(COMMDELAY) + ' --remap __name:=patrol_robot_'+str(i)+' -r __ns:=/robot'+str(i)+' -r /tf:=tf -r /tf_static:=tf_static -p initial_pos:=\"'+iposes.replace(" ", "")+'\"\'')
        elif (ALG_SHORT == 'GBS' or ALG_SHORT == 'SEBS' or ALG_SHORT == 'CBLS'):
            cmd = ('bash -c \'ros2 run patrolling_sim_ros2_gazebo '+ALG+' 0 ' + MAP + ' ' + str(i)+' '+str(NROBOTS)+' --ros-args -p goal_reached_wait:='+str(float(GWAIT)) + ' -p communication_delay:=' +
                   str(COMMDELAY) + ' --remap __name:=patrol_robot_'+str(i) + ' -r __ns:=/robot'+str(i)+' -r /tf:=tf -r /tf_static:=tf_static'+' -p initial_pos:=\"'+iposes.replace(" ", "")+'\"\'')
        else:
            now = datetime.datetime.now()
            dateString = now.strftime("%Y-%m-%d-%H:%M")
            cmd = ('bash -c \'ros2 run patrolling_sim_ros2_gazebo '+ALG+' __name:=patrol_robot_'+str(i)+' '+MAP+' '+str(i)+' --ros-args -p goal_reached_wait:='+str(float(GWAIT)) +
                   ' -p communication_delay:='+str(COMMDELAY) + ' -p initial_pos:=\"'+iposes.replace(" ", "")+'\" -r __ns:=/robot'+str(i)+' -r /tf:=tf -r /tf_static:=tf_static'+'\'')
        print(cmd)
        if (TERM == 'xterm'):
...
class DIP(tk.Frame):
    ...
    def initUI(self):
        ...
        tk.OptionMenu(self, self.map_ddm, *self.map_name_list).grid(sticky=W,
                                                                    row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.robots_ddm, *self.robots_n_list).grid(sticky=W,
                                                                       row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.alg_ddm, *self.algorithm_list).grid(sticky=W,
                                                                     row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.locmode_ddm, *self.locmode_list).grid(sticky=W,
                                                                       row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.navmode_ddm, *self.navmode_list).grid(sticky=W,
                                                                       row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.gwait_ddm, *self.gwait_list).grid(sticky=W,
                                                                   row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.term_ddm, *self.term_list).grid(sticky=W,
                                                                 row=_row, column=1, pady=4, padx=5)
        ...
        tk.OptionMenu(self, self.rviz_ddm, *self.rviz_list).grid(sticky=W,
                                                                 row=_row, column=1, pady=4, padx=5)

        _row = _row + 1

        launchButton = Button(self, text="Start Experiment",
                              command=self.launch_script)
        launchButton.grid(sticky=W, row=_row, column=0, pady=4, padx=5)

        launchButton = Button(self, text="Stop Experiment",
                              command=self.kill_demo)
        launchButton.grid(sticky=W, row=_row, column=1, pady=4, padx=5)

    def launch_script(self):
        self.saveConfigFile()
        _thread.start_new_thread(run_experiment, (self.map_ddm.get(), self.robots_ddm.get(), INITPOS_DEFAULT, self.alg_ddm.get(), self.locmode_ddm.get(
        ), self.navmode_ddm.get(), self.gwait_ddm.get(), COMMDELAY_DEFAULT, self.term_ddm.get(), 0, "false", 1.0, self.rviz_ddm.get()))
...
def main():
    ...
    if (len(sys.argv) == 1):
        ...
        run_experiment(MAP, NROBOTS, INITPOS, ALG_SHORT, LOC_MODE, NAV_MODULE,
                       GWAIT, COMMDELAY, TERM, TIMEOUT, CUSTOM_STAGE, SPEEDUP)
